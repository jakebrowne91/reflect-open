...
const isSupportTicketMetadataUnavailable = (error: unknown) => {
  if (!(error instanceof Error)) return false;

  return error.message.includes("support_ticket_metadata");
};

const supportSourceAliases: Record<string, string[]> = {
  email: ["email", "support_email"],
  slack: ["slack", "slack_bot"],
  emma: ["emma", "ari_gold", "external_agent"],
  sentry: ["sentry"],
};

const expandSupportSources = (sources: string[]) =>
  Array.from(
    new Set(
      sources.flatMap((source) => supportSourceAliases[source] ?? [source]),
    ),
  );
...
export const getByPublicId = async (
  ...
  filters: {
    ...
    reportedAt?: { from?: Date; to?: Date };
    updatedAt?: { from?: Date; to?: Date };
    type: "regular" | "template" | undefined;
  },
) => {
  let cardIds: string[] = [];
  const expandedSources = expandSupportSources(filters.sources);
  const metadataFilterActive =
    expandedSources.length > 0 ||
    filters.supportUserId !== undefined ||
    filters.sourceSystem !== undefined ||
    filters.sourceChannel !== undefined ||
    filters.reportedAt?.from !== undefined ||
    filters.reportedAt?.to !== undefined;
  const relationshipFilterActive =
  ...
  if (relationshipFilterActive) {
    const filteredCards = await db
      ...
      .leftJoin(
        supportTicketMetadata,
        eq(cards.id, supportTicketMetadata.cardId),
      )
      .where(
        and(
          ...
          expandedSources.length > 0
            ? or(
                inArray(supportTicketMetadata.source, expandedSources),
                inArray(supportTicketMetadata.sourceSystem, expandedSources),
              )
            : undefined,
          filters.supportUserId
            ? eq(supportTicketMetadata.userId, filters.supportUserId)
            : undefined,
          filters.sourceSystem
            ? eq(supportTicketMetadata.sourceSystem, filters.sourceSystem)
            : undefined,
          filters.sourceChannel
            ? eq(supportTicketMetadata.sourceChannel, filters.sourceChannel)
            : undefined,
          filters.reportedAt?.from
            ? gte(supportTicketMetadata.reportedAt, filters.reportedAt.from)
            : undefined,
          filters.reportedAt?.to
            ? lte(supportTicketMetadata.reportedAt, filters.reportedAt.to)
            : undefined,
        ),
      );
  ...
  const findBoard = (includeSupportTicketMetadata: boolean) =>
            ...
            cards: {
              ...
              where: and(
                ...
                filters.updatedAt?.from
                  ? gte(
                      sql`COALESCE(${cards.updatedAt}, ${cards.createdAt})`,
                      filters.updatedAt.from,
                    )
                  : undefined,
  ...
  let board: Awaited<ReturnType<typeof findBoard>>;

  try {
    board = await findBoard(true);
  } catch (error) {
    if (!isSupportTicketMetadataUnavailable(error)) throw error;
    board = await findBoard(false);
  }
  ...
  const formattedResult = {
    ...
    lists: board.lists.map((list) => ({
      ...
      cards: list.cards.map((card) => ({
        ...card,
        supportTicketMetadata: card.supportTicketMetadata ?? null,
