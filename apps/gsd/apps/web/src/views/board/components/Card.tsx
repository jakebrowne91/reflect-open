...
const Card = ({
  ...
  const showLastUpdatedAt =
    !!lastUpdatedAt && (visibleSupportFields?.includes("updatedAt") ?? true);
      ...
      {supportDisplayItems.length > 0 && (
        <div className="mt-2 flex flex-wrap gap-1">
          {supportDisplayItems.map((item) => (
            ...
            >
              <span className="flex-shrink-0 text-light-800 dark:text-dark-800">
                {item.label}
              </span>
              <span
                className={twMerge(
            ...
            <div className="flex items-center gap-2">
              ...
              {showLastUpdatedAt && (
                <div className="flex items-center gap-1 text-light-800 dark:text-dark-800">
                  <HiOutlineClock className="h-4 w-4" />
                  <span className="text-[11px]">
                    {format(lastUpdatedAt, "yyyy-MM-dd HH:mm:ss")}
                  </span>
                </div>
              )}
              {hasAttachments && (
