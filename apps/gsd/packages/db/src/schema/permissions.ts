...
export const workspaceRoles = pgTable(
  ...
  (table) => [
    uniqueIndex("gsd_unique_role_per_workspace").on(
      table.workspaceId,
      table.name,
    ),
    index("gsd_workspace_roles_workspace_idx").on(table.workspaceId),
  ],
).enableRLS();
...
export const workspaceRolePermissions = pgTable(
  "gsd_workspace_role_permissions",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
  ...
  (table) => [
    uniqueIndex("gsd_unique_role_permission").on(
      table.workspaceRoleId,
      table.permission,
    ),
    index("gsd_role_permissions_role_idx").on(table.workspaceRoleId),
  ],
).enableRLS();
...
export const workspaceMemberPermissions = pgTable(
  "gsd_workspace_member_permissions",
  {
    id: bigserial("id", { mode: "number" }).primaryKey(),
  ...
  (table) => [
    uniqueIndex("gsd_unique_member_permission").on(
      table.workspaceMemberId,
      table.permission,
    ),
    index("gsd_permission_member_idx").on(table.workspaceMemberId),
  ],
).enableRLS();
