...
import { applyAdminContextToHeaders } from "@kan/auth/admin-context";
...
function closeDbWhenResponseFinishes(
  res: CreateNextContextOptions["res"] | undefined,
  db: dbClient,
) {
  let closed = false;
  const close = () => {
...
export const createInnerTRPCContext = (opts: CreateContextOptions) => {
  return {
    ...
    requestId: opts.headers.get("x-request-id") ?? randomUUID(),
  };
};
...
export const createTRPCContext = async ({
  ...
  closeDbWhenResponseFinishes(res, db);
  const baseAuth = initAuth(db);
  const headers = applyAdminContextToHeaders(
