...
The web app can be built and deployed to Cloudflare Workers with OpenNext:
...
See [CLOUDFLARE.md](./CLOUDFLARE.md) for the production runbook, required
secrets, and DNS cutover checklist.
...
| `POSTGRES_POOL_MAX`                       | PostgreSQL connection pool size                           | For external database                       | `5`                                                                  |
| `POSTGRES_SSL`                            | Force or disable PostgreSQL TLS                           | For external database                       | `true`                                                               |
| `POSTGRES_SSL_REJECT_UNAUTHORIZED`        | Verify PostgreSQL custom CA chains                        | For external database                       | `true`                                                               |
...
| `S3_REGION`                               | S3 storage region                                         | For file uploads                            | `<project-region>`                                                   |
| `S3_ENDPOINT`                             | S3 endpoint URL                                           | For file uploads                            | `https://<project-ref>.storage.supabase.co/storage/v1/s3`            |
...
| `NEXT_PUBLIC_STORAGE_URL`                 | Storage service URL                                       | For file uploads                            | `https://<project-ref>.storage.supabase.co/storage/v1/object/public` |
| `NEXT_PUBLIC_STORAGE_DOMAIN`              | Storage domain name                                       | For file uploads                            | `<project-ref>.storage.supabase.co`                                  |
| `NEXT_PUBLIC_USE_VIRTUAL_HOSTED_URLS`     | Use virtual-hosted style URLs (bucket.domain.com)         | For file uploads (optional)                 | `true`                                                               |
| `NEXT_PUBLIC_AVATAR_BUCKET_NAME`          | S3 bucket name for avatars                                | For file uploads                            | `gsd-avatars`                                                        |
| `NEXT_PUBLIC_ATTACHMENTS_BUCKET_NAME`     | S3 bucket name for attachments                            | For file uploads                            | `gsd-attachments`                                                    |
