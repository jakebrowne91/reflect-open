...
import { supportTicketMetadata } from "./supportTicketMetadata";
...
export const activityTypeEnum = pgEnum("gsd_card_activity_type", activityTypes);
export const cardPriorityEnum = pgEnum("gsd_card_priority", [
  "urgent",
  "high",
  "medium",
  "low",
...
export const cards = pgTable(
  ...
  (table) => [
    index("gsd_card_list_number_idx").on(table.listId, table.cardNumber),
    index("gsd_cards_title_trgm_idx").using(
      "gin",
...
export const cardsRelations = relations(cards, ({ one, many }) => ({
  ...
  supportTicketMetadata: one(supportTicketMetadata),
}));

export const cardActivities = pgTable("gsd_card_activity", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  publicId: varchar("publicId", { length: 12 }).notNull().unique(),
...
export const comments = pgTable("gsd_card_comments", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  publicId: varchar("publicId", { length: 12 }).notNull().unique(),
...
export const cardAttachments = pgTable("gsd_card_attachment", {
  id: bigserial("id", { mode: "number" }).primaryKey(),
  publicId: varchar("publicId", { length: 12 }).notNull().unique(),
  cardId: bigint("cardId", { mode: "number" })
    .notNull()
