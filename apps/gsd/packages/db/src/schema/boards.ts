...
export const boards = pgTable(
  ...
  (table) => [
    index("gsd_board_is_archived_idx").on(table.isArchived),
    index("gsd_board_visibility_idx").on(table.visibility),
    index("gsd_board_type_idx").on(table.type),
    index("gsd_board_source_idx").on(table.sourceBoardId),
    index("gsd_boards_name_trgm_idx").using(
      "gin",
      table.name.op("gin_trgm_ops"),
    ),
    uniqueIndex("gsd_unique_slug_per_workspace")
...
export const userBoardFavorites = pgTable(
  ...
  (table) => ({
    ...
    userIdx: index("gsd_user_board_favorite_user_idx").on(table.userId),
    boardIdx: index("gsd_user_board_favorite_board_idx").on(table.boardId),
  }),
);
