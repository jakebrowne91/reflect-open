...
export default function CheckboxDropdown({
  ...
  const selectedGroupConfig = groups?.find((g) => g.key === selectedGroup);
    ...
    >
      <>
        ...
        >
          <Menu.Items
            className={twMerge(
              "mt-2s absolute z-50 origin-top-left rounded-md border-[1px] border-light-200 bg-light-50 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none dark:border-dark-500 dark:bg-dark-200",
              menuWidthClass[menuWidth],
          ...
          >
            <div className="max-h-[350px] overflow-y-auto p-1">
              {!selectedGroup ? (
                ...
                <>
                  {selectedGroupConfig?.panel ??
                    (selectedGroupConfig?.items &&
                      renderMenuItems(
                        selectedGroupConfig.items,
                        selectedGroup,
                      ))}
