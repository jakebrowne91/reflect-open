...
export const cardRouter = createTRPCRouter({
  ...
  byId: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const supportTicketMetadata = await supportTicketMetadataRepo.getByCardId(
        ctx.db,
        result.id,
      );
      ...
      return {
        ...result,
        supportTicketMetadata: supportTicketMetadata
          ? {
              externalId: supportTicketMetadata.externalId,
              source: supportTicketMetadata.source,
              sourceCaseKey: supportTicketMetadata.sourceCaseKey,
              supportCaseId: supportTicketMetadata.supportCaseId,
              sourceEventId: supportTicketMetadata.sourceEventId,
              sourceSystem: supportTicketMetadata.sourceSystem,
              userId: supportTicketMetadata.userId,
              emmaUserId: supportTicketMetadata.emmaUserId,
              email: supportTicketMetadata.email,
              customerName: supportTicketMetadata.customerName,
              issueCategory: supportTicketMetadata.issueCategory,
              reportedAt: supportTicketMetadata.reportedAt,
              sourceChannel: supportTicketMetadata.sourceChannel,
              provider: supportTicketMetadata.provider,
              providerThreadId: supportTicketMetadata.providerThreadId,
              providerMessageId: supportTicketMetadata.providerMessageId,
              mailbox: supportTicketMetadata.mailbox,
              ariSessionId: supportTicketMetadata.ariSessionId,
              ariSessionUrl: supportTicketMetadata.ariSessionUrl,
              repoFullName: supportTicketMetadata.repoFullName,
            }
          : null,
  ...
  listFeatureRequests: protectedProcedure
    .meta({
      openapi: {
        summary: "List feature request cards",
        method: "GET",
        path: "/workspaces/{workspacePublicId}/feature-requests",
        description:
          "Retrieves support cards categorised as feature requests for a given workspace",
        tags: ["Cards"],
        protect: true,
      },
    })
    ...
    .output(z.array(featureRequestListItemSchema))
    .query(async ({ ctx, input }) => {
      const userId = ctx.user?.id;
  ...
  listSentEmails: protectedProcedure
    .meta({
      openapi: {
        summary: "List sent support emails",
        method: "GET",
        path: "/workspaces/{workspacePublicId}/sent-emails",
        description:
          "Retrieves outbound customer-facing support emails recorded by the support agent",
        tags: ["Cards"],
        protect: true,
      },
    })
    ...
    .output(z.array(sentEmailListItemSchema))
    .query(async ({ ctx, input }) => {
      const userId = ctx.user?.id;
      ...
      const messages =
        await supportTicketMetadataRepo.listOutboundEmailMessages(ctx.db, {
          limit: input.limit,
        });
      ...
      return messages.map((message) => ({
        ...
        cardPublicId:
          message.card && !message.card.deletedAt
            ? message.card.publicId
            : null,
        cardTitle:
          message.card && !message.card.deletedAt ? message.card.title : null,
        sentAt: message.createdAt,
      }));
    }),
  listSpamRecords: protectedProcedure
    .meta({
      openapi: {
        summary: "List card-less support email records",
        method: "GET",
        path: "/workspaces/{workspacePublicId}/spam-records",
        description:
          "Retrieves inbound support emails recorded without a card (spam, non-issues and unmatched emails)",
        tags: ["Cards"],
        protect: true,
      },
    })
    ...
    .output(z.array(spamRecordListItemSchema))
    .query(async ({ ctx, input }) => {
      const userId = ctx.user?.id;
      ...
      const messages =
        await supportTicketMetadataRepo.listCardlessEmailMessages(ctx.db, {
          limit: input.limit,
        });
      ...
      return messages.map((message) => ({
        ...
        subject: message.subject,
        classification:
          (message.classification as Record<string, unknown> | null) ?? null,
        processingStatus: message.processingStatus,
  ...
  update: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (movedToNewList && newList) {
        const fromStatus = supportStatusFromListName(existingCard.list.name);
        const toStatus = supportStatusFromListName(newList.name);
        if (fromStatus && toStatus && fromStatus !== toStatus) {
          const supportMetadata = await supportTicketMetadataRepo.getByCardId(
            ctx.db,
            result.id,
          );
          if (supportMetadata) {
            const event = await supportTicketMetadataRepo.createEventIfNew(
              ctx.db,
              {
                cardId: result.id,
                source: "gsd",
                sourceEventId: `status:${input.cardPublicId}:manual:${existingCard.listId}:${newList.id}:${Date.now()}`,
                sourceCaseKey: supportMetadata.sourceCaseKey,
                eventType: "status_changed",
                status: toStatus,
                summary: `Card moved from ${existingCard.list.name} to ${newList.name}.`,
                payload: {
                  cardPublicId: input.cardPublicId,
                  fromStatus,
                  toStatus,
            ...
            if (event) {
              await postSupportTicketStatusSlackUpdate({
                eventId: input.cardPublicId,
                source: supportMetadata.source,
                status: toStatus,
                title: result.title,
                summary: `Card moved from ${existingCard.list.name} to ${newList.name}.`,
                ticketUrl: buildRetrogradeGsdCardUrl(input.cardPublicId),
              });
            }
          }
        }
